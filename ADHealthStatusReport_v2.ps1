...
Foreach ($DC in $DomainControllers) {
    ...
    Start-Job -ScriptBlock { Get-Service -ComputerName $($args[0]) -Name "netlogon","NTDS","DNS" -ErrorAction SilentlyContinue } -ArgumentList $DC -Name "$DC - Services"

    Start-Job -scriptblock { DCDIAG /test:Advertising /test:NetLogons /test:Replications /test:Services /test:FSMOCheck /s:$($args[0])} -ArgumentList $DC -Name "$DC - DCDIAG"
}
...
Foreach ($DC in $DomainControllers) {
    ...
    IF ( $Status -eq 0 ) {
        ...
        $ServiceStatus = Get-Job -Name "$DC - Services"
        ...
        $DiagCheck = Get-Job -Name "$DC - DCDIAG"
